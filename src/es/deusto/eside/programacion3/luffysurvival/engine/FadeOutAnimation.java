 ...
 */
public class FadeOutAnimation implements Renderable {
	...
	private final Logger logger = LoggerFactory.getLogger(FadeOutAnimation.class);
	 ...
	 */
	public FadeOutAnimation(Image[] frames, int duration) {
	 ...
	 */
	public FadeOutAnimation(Image[] frames, int[] durations) {
	 ...
	 */
	public FadeOutAnimation(boolean autoUpdate) {
		currentFrame = 0;
	 ...
	 * current frame will be caculated based on the time between renders
	 */
	public FadeOutAnimation(Image[] frames, int duration, boolean autoUpdate) {
	 ...
	 * current frame will be caculated based on the time between renders
	 */
	public FadeOutAnimation(Image[] frames, int[] durations, boolean autoUpdate) {
		this.autoUpdate = autoUpdate;
	 ...
	 */
	public FadeOutAnimation(SpriteSheet frames, int duration) {
	 ...
	 */
	public FadeOutAnimation(SpriteSheet frames, int x1, int y1, int x2, int y2, boolean horizontalScan, int duration, boolean autoUpdate) {
		this.autoUpdate = autoUpdate;
	 ...
	 * @param duration The duration each frame should be displayed for
	 */
	public FadeOutAnimation(SpriteSheet ss, int[] frames, int[] duration){
		spriteSheet = ss;
	    int x = -1;
	    int y = -1;
	...
	public void draw(float x,float y,float width,float height, Color col) {
		...
		if (nextChange > frame.duration * 3/4) {
			frame.image.setAlpha((   4000F - nextChange)/ 1000F);
		} else if (nextChange <= frame.duration * 3/4 && nextChange >= frame.duration * 1/4) {
			frame.image.setAlpha(1.0F);

		} else {
			frame.image.setAlpha(1F-(frame.duration * 1/4 - nextChange )/ 1000F);
	 ...
	 */
	public FadeOutAnimation copy() {
		FadeOutAnimation copy = new FadeOutAnimation();
